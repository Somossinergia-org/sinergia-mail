...
export default function FloatingAgent({ open, onClose, onOpen }: Props) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-20 lg:bottom-6 right-4 lg:right-6 z-30 w-14 h-14 rounded-full bg-gradient-to-br from-sinergia-500 to-purple-500 shadow-[0_0_24px_rgba(168,85,247,0.45)] hover:shadow-[0_0_36px_rgba(168,85,247,0.65)] flex items-center justify-center hover:scale-110 transition-all relative"
      >
        <span className="absolute inset-0 rounded-full bg-purple-500/40 animate-ping opacity-40" aria-hidden />
        <Bot className="w-6 h-6 text-white relative z-10" />
        {messages.length > 0 && (
          <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center shadow-[0_0_8px_rgba(239,68,68,0.6)]">
  ...
  return (
    <div
      ...
      onDrop={onDrop}
    >
      <div className={`ai-neon-frame ai-chat-surface flex-1 lg:flex-none lg:h-full lg:rounded-2xl shadow-2xl flex flex-col overflow-hidden ${sending || listening ? "is-active" : ""}`}>
        {/* Header */}
        <div className="relative z-10 flex items-center gap-3 px-4 py-3 border-b border-[var(--border)]/60 backdrop-blur-md">
          <span className={`ai-orb ${sending ? "is-speaking" : ""}`} aria-hidden />
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {sending && (
            <div className="flex items-start gap-2">
              <div className="w-7 h-7 rounded-full bg-purple-500/15 border border-purple-500/30 flex items-center justify-center flex-shrink-0" style={{ boxShadow: "0 0 12px rgba(168,85,247,0.4)" }}>
                <Bot className="w-3.5 h-3.5 text-purple-400" />
              </div>
              <div className="ai-bubble rounded-2xl px-3 py-2">
    ...
    <div className={`flex gap-2 ${isUser ? "flex-row-reverse" : ""}`}>
      <div
        className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 border ${
          isUser
            ? "bg-sinergia-600/20 text-sinergia-300 border-sinergia-500/30"
            : "bg-purple-500/15 text-purple-300 border-purple-500/30"
        }`}
        style={{ boxShadow: isUser ? "0 0 10px rgba(99,102,241,0.25)" : "0 0 10px rgba(168,85,247,0.25)" }}
      >
        {isUser ? <User className="w-3.5 h-3.5" /> : <Bot className="w-3.5 h-3.5" />}
      </div>
      <div className="max-w-[78%] space-y-1.5">
        <div className={`rounded-2xl px-3 py-2 text-sm leading-relaxed ${isUser ? "user-bubble" : "ai-bubble"}`}>
          {message.attachment && (
